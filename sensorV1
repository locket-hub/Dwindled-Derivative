...
def wait_for_beam_break():
    """Block until the beam is broken (input goes LOW), then wait for it to reset."""
    ...
    # Optional: wait for beam to be unbroken again before next photo
    print("Waiting for beam to be unbroken...")
    while not GPIO.input(BEAM_PIN):  # LOW = still broken
        time.sleep(0.01)
    print("Beam unbroken, ready for next photo.")
...
# ------------------------
# PHOTO CLASS FOR 10 SHOTS (SENSOR GATED)
# ------------------------
